...
namespace arduinobot_cpp_examples
{
    ...
    {
    public:
        explicit SimpleActionServer(const rclcpp::NodeOptions options = rclcpp::NodeOptions()) : Node("simple_action_sever", options)
        {
            ...
            RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Starting an Action Server");
        }
    ...
    private:
        ...
        {
            RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Recieved Goal Request with order:" << goal->order);
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Execute Goal");
