...
class AnglesConverter(Node):
    ...
    def eulerToQuaternionCallback(self, req, res):
        self.get_logger().info("Request to convert Euler angles roll: %f, pitch: %f , yaw: %f, into Quaternion." % (req.roll, req.pitch,req.yaw))
        (res.x, res.y, res.z, res.w)= quaternion_from_euler(req.roll, req.pitch, req.yaw)
        self.get_logger().info("Corresponding Quaternions x: %f, y: %f, z: %f, w: %f" % (res.x, res.y, res.z, res.w))
        return res
        

    def quaternionToEulerCallback(self, req, res):
        self.get_logger().info("Request to convert Quaternion angles x: %f, y: %f , z: %f, w: %f, into Euler." % (req.x, req.y, req.z, req.w))
